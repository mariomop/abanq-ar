...
class oficial extends interna 
{
	...
	function insertarCiudad() {
		return this.ctx.oficial_insertarCiudad();
	}
	function quitarCiudad() {
		return this.ctx.oficial_quitarCiudad();
	}
}
//// OFICIAL /////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////
...
function interna_init()
{
	...
	connect (this.child("tbIncluirCiu"), "clicked()", this, "iface.insertarCiudad()");
	connect (this.child("tbExcluirCiu"), "clicked()", this, "iface.quitarCiudad()");
	...
	this.child("tdbCiudades").cursor().setMainFilter("codzona is null OR codzona = ''");
}
...
function oficial_insertarCiudad()
{
	var util:FLUtil;
	...
	var curTab:FLSqlCursor = this.child("tdbCiudades").cursor();
	if (!curTab.isValid())
		return;
	
	var idCiudad:String = curTab.valueBuffer("idCiudad");
	curTab = new FLSqlCursor("ciudades");
	curTab.select("idciudad = " + idCiudad);
	if (!curTab.first())
		return;
	curTab.setModeAccess(curTab.Edit);
	curTab.refreshBuffer();
	...
	this.child("tdbCiudades").refresh();
	this.child("tdbCiudadesSel").refresh();
}

function oficial_quitarCiudad()
{
	var util:FLUtil;

	var curTab:FLSqlCursor = this.child("tdbCiudadesSel").cursor();
	if (!curTab.isValid())
		return;
