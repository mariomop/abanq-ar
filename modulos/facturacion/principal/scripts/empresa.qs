...
function silixExtensiones_validateForm():Boolean
{
	...
	// No pueden activarse las cuentas de caja si no está cargado el módulo de Cuentas
	if (!sys.isLoadedModule("flfactcuentas") && (this.cursor().valueBuffer("cuentascajaintegrada") == true)) {
		MessageBox.warning(util.translate("scripts", "No pueden activarse las cuentas de caja si no está cargado el módulo de Cuentas"), MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton);
		return false;
	}
