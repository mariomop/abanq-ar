...
/////////////////////////////////////////////////////////////////
//// DESARROLLO /////////////////////////////////////////////////
class head extends cantLineas {
    function head( context ) { cantLineas ( context ); }
}
//// DESARROLLO /////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////
	...
	switch (fN) {
		...
		case "codcliente": {
			if (cursor.valueBuffer("codcliente") && cursor.valueBuffer("codcliente") != "") {
				var respIva = util.sqlSelect("clientes", "regimeniva", "codcliente = '" + cursor.valueBuffer("codcliente") + "'");
				// Este switch da por supuesto que el emisor de la factura es un Responsable Inscripto, por lo cual emite facturas A o B dependiendo del cliente al que vende.
				switch ( respIva ) {
...
function numeroSecuencia_calculateField(fN:String):String
{
	...
	switch (fN) {
		case "numerosecuencia": {
			var idSec:Number = util.sqlSelect("secuenciasejercicios", "id", "codejercicio = '" + flfactppal.iface.pub_ejercicioActual() + "' AND codserie = '" + this.child("fdbCodSerie").value() + "'");
			if (!idSec) {
				valor = 0;
				break;
			}
