...
function pieDocumento_commonCalculateField(fN:String, cursor:FLSqlCursor):String
{
	...
	switch (fN) {
		case "totalpie": {
			var totalLineas:Number = parseFloat(util.sqlSelect("piepedidosprov", "SUM(totalinc)", "idpedido = " + cursor.valueBuffer("idpedido") + " AND coniva = false"));
		...
		case "neto": {
			var netoPie:Number = parseFloat(util.sqlSelect("piepedidosprov", "SUM(totalinc)", "idpedido = " + cursor.valueBuffer("idpedido") + " AND coniva = true"));
			valor = this.iface.__commonCalculateField(fN, cursor);
			valor += netoPie;
			valor = parseFloat(util.roundFieldValue(valor, "pedidosprov", "neto"));
			break;
