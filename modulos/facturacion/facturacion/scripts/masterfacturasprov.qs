...
function ordenCampos_init()
{
	...
	var orden:Array = [ "codigo", "tipoventa", "editable", "numproveedor", "nombre", "neto", "totaliva", "recfinanciero", "total", "coddivisa", "tasaconv", "totaleuros", "fecha", "hora", "codserie", "numero", "codejercicio", "codalmacen", "codpago", "codproveedor", "cifnif", "automatica", "rectificada", "decredito", "dedebito", "codigorect", "idusuario", "observaciones" ];
...
function pieDocumento_commonCalculateField(fN:String, cursor:FLSqlCursor):String
{
	...
	switch (fN) {
		case "totalpie": {
			var totalLineas:Number = parseFloat(util.sqlSelect("piefacturasprov", "SUM(totalinc)", "idfactura = " + cursor.valueBuffer("idfactura") + " AND coniva = false"));
		...
		case "neto": {
			var netoPie:Number = parseFloat(util.sqlSelect("piefacturasprov", "SUM(totalinc)", "idfactura = " + cursor.valueBuffer("idfactura") + " AND coniva = true"));
			valor = this.iface.__commonCalculateField(fN, cursor);
			valor += netoPie;
			valor = parseFloat(util.roundFieldValue(valor, "facturasprov", "neto"));
			break;
