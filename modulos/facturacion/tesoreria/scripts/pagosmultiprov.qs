...
function interna_init()
{
	...
	this.child("fdbCodProveedor").setFocus();
}
...
function numeroSecuencia_acceptedForm()
{
	...
	if ( this.iface.modoAcceso == this.cursor().Insert ) {
		...
		if (cursorSecuencias.next()) {
			...
			cursorSecs.select( "id = " + idSec + " AND nombre = 'npagomultiprov'" );
			if (cursorSecs.next()) {
...
function periodosFiscales_validateForm():Boolean
{
	...
	if (!codPeriodo) {
		MessageBox.warning(util.translate("scripts", "Debe indicar el período fiscal al que se imputa la orden"), MessageBox.Ok, MessageBox.NoButton);
		return false;
	}
	var estado:String = util.sqlSelect("periodos", "estado", "codperiodo = '" + codPeriodo + "'");
	if (estado == "CERRADO") {
		MessageBox.warning(util.translate("scripts", "No puede incluirse la orden en un período fiscal cerrado.\nDebe crear un nuevo período fiscal para el ejercicio actual."), MessageBox.Ok, MessageBox.NoButton);
		return false;
	}
