...
class pagosMultiples extends oficial {
	...
	function filtroRecibosCli():String {
		return this.ctx.pagosMultiples_filtroRecibosCli();
	}
...
function interna_init()
{
	...
	if (this.iface.contabActivada) {
		...
		this.child("lblResAsientos").text = util.translate("scripts", "Existe un asiento por pago de comprobante. A continuación se muestra un acumulado de las partidas de todos los asientos asociados a los comprobantes del recibo de cobro.");
...
function interna_validateForm():Boolean
{
	...
	\end */
	if (!util.sqlSelect("pagosdevolcli", "idpagodevol", "idpagomulti = " + cursor.valueBuffer("idpagomulti"))) {
		MessageBox.warning(util.translate("scripts", "El recibo de cobro debe tener al menos un comprobante."), MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton);
		return false;
	}

	/** \C El pago múltiple debe tener al menos un pago
	\end */
	if (!util.sqlSelect("pagosdevolpagosmulticli", "idpagopagomulti", "idpagomulti = " + cursor.valueBuffer("idpagomulti"))) {
		MessageBox.warning(util.translate("scripts", "El recibo de cobro debe tener al menos un pago."), MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton);
		return false;
	}
...
function interna_calculateField(fN:String):String
{
	...
	switch (fN) {
		...
		case "total": {
			res = util.sqlSelect("reciboscli", "SUM(importe)", "idrecibo IN (SELECT idrecibo FROM pagosdevolcli WHERE idpagomulti = " + cursor.valueBuffer("idpagomulti") + ")");
			break;
		}
		case "importeeuros": {
...
function pagosMultiples_agregarRecibo():Boolean
{
	...
	if (!cursor.valueBuffer("codcliente")) {
		MessageBox.warning(util.translate("scripts", "Debe indicar un cliente"), MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton);
		return;
	}
...
function pagosMultiples_filtroRecibosCli():String
{
	var cursor:FLSqlCursor = this.cursor();
	return "estado = 'Pendiente' AND fecha <= '" + cursor.valueBuffer("fecha") + "' AND codcliente = '" + cursor.valueBuffer("codcliente") + "'";
}
...
function pagosMultiples_excluirReciboPagoMulti(idRecibo:String, idPagoMulti:String):Boolean
{
	...
	if (util.sqlSelect("pagosmulticli", "codcuenta", "idpagomulti = " + idPagoMulti)) {
		...
		if (!cuentaValida) {
			...
			MessageBox.warning(util.translate("scripts", "La cuenta bancaria del comprobante %1 no es una cuenta válida del cliente.\nCambie o borre la cuenta antes de excluir el comprobante del recibo de cobro.").arg(codRecibo), MessageBox.Ok, MessageBox.NoButton);
			return false;
		}
	}
...
function pagosMultiples_asociarReciboPagoMulti(idRecibo:String, curPagoMulti:FLSqlCursor):Boolean
{
	...
	if (util.sqlSelect("reciboscli", "coddivisa", "idrecibo = " + idRecibo) != curPagoMulti.valueBuffer("coddivisa")) {
		MessageBox.warning(util.translate("scripts", "No es posible incluir el comprobante.\nLa divisa del comprobante y del recibo de cobro deben ser la misma."), MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton);
		return;
	}
	...
	if (this.iface.curPagosDev.last()) {
		if (util.daysTo(this.iface.curPagosDev.valueBuffer("fecha"), fecha) < 0) {
			var codRecibo:String = util.sqlSelect("reciboscli", "codigo", "idrecibo = " + idRecibo);
			MessageBox.warning(util.translate("scripts", "Existen pagos con fecha igual o porterior a la del recibo de cobro para el comprobante %1").arg(codRecibo), MessageBox.Ok, MessageBox.NoButton);
			return false;
		}
	}
...
function tipoVenta_calculateField(fN:String):String
{
	...
	switch (fN) {
		...
		case "codigo": {
			valor = formpagosmulticli.iface.pub_commonCalculateField(fN, cursor);
			break
		}
		default: {
			valor = this.iface.__calculateField(fN);
