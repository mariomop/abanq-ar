...
class oficial extends interna {
	...
	function rellenarDatos(idImpresion:String) {
		return this.ctx.oficial_rellenarDatos(idImpresion);
	}
...
// 	var datos:FLDomDocument = this.iface.rellenarDatos(idImpresion);
	
	this.iface.rellenarDatos(idImpresion);
...
/*	try {
		/// Con XML
debug("lanzando XML");
		rptViewer.setReportData(datos);
	} catch (e) {
		/// Con tabla
debug("lanzando tabla");
		rptViewer.setReportData(q);
	}*/
...
function oficial_rellenarDatos(idImpresion:String):FLDomDocument
{
	var util:FLUtil = new FLUtil;
	var cursor:FLSqlCursor = this.cursor()
...
// 		nodoRow = this.iface.dameNodoXML(xmlKD.firstChild(), "Row[@co_i_balancesis_buffer.codsubcuenta=" + codSubcuenta + "]");
		...
		if (curTab.first()/* && nodoRow*/) {
			...
			curTab.setValueBuffer("debe", parseFloat(curTab.valueBuffer("debe")) + parseFloat(q.value(4)));
			curTab.setValueBuffer("haber", parseFloat(curTab.valueBuffer("haber")) + parseFloat(q.value(5)));
			curTab.setValueBuffer("saldo", parseFloat(curTab.valueBuffer("saldo")) + parseFloat(q.value(6)));
			/// Con XML
// 			eRow = nodoRow.toElement();
// 			eRow.setAttribute("co_i_balancesis_buffer.debe", parseFloat(eRow.attribute("co_i_balancesis_buffer.debe")) + parseFloat(q.value(4)));
// 			eRow.setAttribute("co_i_balancesis_buffer.haber", parseFloat(eRow.attribute("co_i_balancesis_buffer.haber")) + parseFloat(q.value(5)));
// 			eRow.setAttribute("co_i_balancesis_buffer.saldo", parseFloat(eRow.attribute("co_i_balancesis_buffer.saldo")) + parseFloat(q.value(6)));
		} else {
			/// Con tabla
...
// 			eRow.setAttribute("co_i_balancesis_buffer.debe", q.value(4));
// 			eRow.setAttribute("co_i_balancesis_buffer.haber", q.value(5));
// 			eRow.setAttribute("co_i_balancesis_buffer.saldo", q.value(6));
