...
function oficial_validarSubcuenta()
{
	...
	if (codCuenta && codCuenta != "" && codSubcuenta && codSubcuenta != "") {
		...
		if (this.iface.longSubcuenta > 8) {
			if (codCuenta.length == 3) {
				var cuartoDigito:String = codSubcuenta.charAt(3);
				if (cuartoDigito != "0") {
					MessageBox.warning(util.translate("MetaData", "Si el código de cuenta tiene tres caracteres el cuarto dígito de la subcuenta debe ser 0.\nEsto es necesario para evitar incongruencias en subcuentas que pueden descender de dos o más cuentas."), MessageBox.Ok, MessageBox.NoButton);
					return false;
				}
			}
		}
